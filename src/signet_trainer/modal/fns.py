...
@app.function(
    ...
    # ⛔ LTX ``sample`` (below) is DELIBERATELY EXCLUDED from ``single_use_containers`` ONLY — a
    #   render is not resumable so a fresh container per retry buys nothing extra. It is NOT excluded
    #   from retries: ``sample`` carries its OWN ``max_retries=3`` (AUDIT-#1 (d), on its decorator
    #   below) so a preempted render self-heals rather than being a total loss — that retry budget
    #   is the real ceiling on a render's worst-case spend (issue #33 finding 4; a prior version of
    #   this comment claimed "excluded from both", which was true only for ``h3_sample`` below).
...
# validator), so the in-fn ``NotImplementedError`` branch is defense-in-depth only. ``destination=
# 'local'`` is likewise fail-fasted at load when enabled — a Modal container's filesystem is not
# durable (issue #23 finding 1) — and ``backup.enabled=False`` is checked FIRST in both functions
# (issue #33 finding 1), so a disabled block never reaches either destination branch at all. Both are
# ADDITIVE
...
def backup_sync(config_text: str) -> str:
    ...
    read from config, never printed. An ENABLED 'local' or 'cloud' destination is rejected at config
    load (only 'hf' is durable+wired); the in-fn branches below are defense-in-depth backstops only,
    reachable only if that validator were bypassed.
    ...
    # an enabled destination='local' or 'cloud' block already fail-fasted at the entrypoint gate
    # before dispatch (both are refused at config load; only 'hf' is durable+wired here).
    cfg = load_config_from_text(config_text)
    backup = cfg.backup

    # Issue #33 finding 1: backup.enabled is the DOCUMENTED master gate for all backup activity
    # (schema.py's BackupConfig.enabled), but no dispatch-side code read it — the entrypoint's
    # pre-approval refusal (main(), below) is the primary defense, but it is possible to call this
    # fn directly (or bypass that gate), so this is the defense-in-depth backstop: a disabled block
    # is a $0 no-op REGARDLESS of what destination/fields it carries, matching "ALL backup activity
    # is gated on this" literally rather than only at the validator that gates itself.
    if not backup.enabled:
        return "[backup_sync] no-op: backup.enabled=False (BK-01 master gate OFF) — nothing mirrored."
    ...
    if backup.destination == "hf":
        ...
        try:
            ...
            # issue #23 finding 3: narrowed from a bare `except Exception`. This is the ONLY signal
            # that legitimately means "empty/unknown remote" — the tree (or repo) does not exist yet
            # at this prefix. `EntryNotFoundError` also catches its subclass `RemoteEntryNotFoundError`
            # (the one list_repo_tree actually raises for a missing path_in_repo folder), and
            # `RepositoryNotFoundError` covers a not-yet-existing repo as a belt-and-suspenders case
            # even though create_repo(exist_ok=True) already ran above. Every OTHER failure — an auth
            # scope that permits write but not list, a rate limit, a transient 5xx — must PROPAGATE
            # loudly rather than being rendered as a known-empty remote (a durability verifier must
            # never mistake "I don't know" for "there is nothing here").
            remote_names = set()
    ...
    # UNREACHABLE via the gate (defense-in-depth only) WHEN ENABLED: 09.1-07's load-time validator
    # fail-fasts an enabled destination='cloud' block, and the in-body load_config_from_text above
    # re-ran it. When DISABLED, the backup.enabled check above already returned a no-op before this
    # point (issue #33 finding 1) — so this branch is unreachable in either case, not "only for
    # enabled=True" as an earlier version of this comment implied. A future refactor that bypassed
    # load-time validation or the enabled check would hit this backstop with the same message.
    raise NotImplementedError(
        "backup.destination='cloud' is schema-ready but not yet implemented — only 'hf' is wired "
        "(and durable) this phase. This branch is unreachable (the config validator fail-fasts an "
        "enabled cloud block at load); it exists only as a defense-in-depth backstop."
    )
...
def restore(config_text: str) -> str:
    ...
    token comes from the Modal secret env ONLY — never read from config, never printed. An ENABLED
    'cloud' destination is rejected at config load; the in-fn branch is a defense-in-depth backstop.
    """
    from pathlib import Path  # noqa: PLC0415
    ...
    # Issue #33 finding 1 (restore's twin of backup_sync's gate, see there for the full rationale):
    # backup.enabled is the master gate for ALL backup activity, INCLUDING restore. A disabled block
    # must never attempt a snapshot_download/copy — this is the defense-in-depth backstop behind the
    # entrypoint's pre-approval refusal.
    if not backup.enabled:
        return "[restore] no-op: backup.enabled=False (BK-01 master gate OFF) — nothing rehydrated."
    ...
    # issue #23 finding 2: snapshot the destination BEFORE the copy so the summary can report the
    # DELTA this call actually produced, not a listing of the destination taken AFTER the copy (which
    # counts every dir already on the Volume — including ones this call never touched — as "restored").
    before_names = (
        {p.name for p in restored_root.iterdir() if p.is_dir()} if restored_root.exists() else set()
    )
    ...
    # issue #23 finding 2: report the DELTA (after - before), not a bare post-copy listing — a
    # restore that matched/copied nothing must never be reported as "restored N dir(s)" just because
    # N dirs already sat on the Volume before this call ran.
    after_names = (
        {p.name for p in restored_root.iterdir() if p.is_dir()} if restored_root.exists() else set()
    )
    new_names = sorted(after_names - before_names)
    already_present = len(before_names & after_names)
    print(
        f"[restore] rehydrated {len(new_names)} NEW checkpoint dir(s) under {restored_root} "
        f"({new_names}); {already_present} already present pre-restore. Committed the Volume. "
        "Additive — no existing checkpoint deleted."
    )
    return (
        f"[restore] restored {len(new_names)} dir(s) (new); {already_present} already present; "
        "committed signe-trainer-checkpoints."
    )
